...
def process_midi_files(in_files_path: str = "../data/midi/quantized") -> Dict[str, ndarray]:
    ...
    for file_path in natsort.natsorted(glob.glob(in_files_path + "/*.mid", recursive=True)):
        ...
        samples[os.path.splitext(ntpath.basename(file_path))[0]] = slices
...
def save_as_numpy(samples: Dict[str, ndarray], path: str = "data/numpy"):
    for name, sample in samples:
        file_path = path + "/{}".format(name)
